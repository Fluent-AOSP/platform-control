...
M0–M3 are complete: the upstream AOSP checkout is revision-locked, the local product builds reproducibly through documented compatibility patches, and the same-build Cuttlefish evidence loop is stable. M4 has produced a validated Windows 11-aligned Quick Settings composition and scoped Windows-blue, cool-neutral Acrylic-like color/material hierarchy through local AOSP commit `9f104c3c949e`, including explicit opaque no-blur fallbacks. See [ROADMAP.md](ROADMAP.md), the [Quick Settings foundation](docs/design/quick-settings-foundation.md), and [`patches/`](patches/README.md).
